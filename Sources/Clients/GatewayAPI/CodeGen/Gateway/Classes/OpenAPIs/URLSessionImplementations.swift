...
public typealias GatewayAPIChallengeHandler = (URLSession, URLSessionTask, URLAuthenticationChallenge) -> (URLSession.AuthChallengeDisposition, URLCredential?)
...
private var challengeHandlerStore = SynchronizedDictionary<Int, GatewayAPIChallengeHandler>()
...
open class URLSessionRequestBuilder<T>: RequestBuilder<T> {
	 ...
	 */
	public var taskDidReceiveChallenge: GatewayAPIChallengeHandler?
	...
	@discardableResult
	override open func execute(_ apiResponseQueue: DispatchQueue = GatewayAPI.apiResponseQueue, _ completion: @escaping (_ result: Swift.Result<Response<T>, ErrorResponse>) -> Void) -> RequestTask {
	...
	open func buildHeaders() -> [String: String] {
		...
		for (key, value) in GatewayAPI.customHeaders {
			httpHeaders[key] = value
		}
		return httpHeaders
	}
		...
		case is URL.Type:
			do {
				...
				if let headerFileName = getFileName(fromContentDisposition: httpResponse.allHeaderFields["Content-Disposition"] as? String) {
					...
					requestPath = requestPath.appending("/tmp.Gateway.\(UUID().uuidString)")
				}
