...
public struct ResourcesList: FeatureReducer, Sendable {
	...
	public func reduce(into state: inout State, internalAction: InternalAction) -> Effect<Action> {
		...
		case let .resourceLoaded(resource, newAsset):
			state.loadedResources.append(.init(iconURL: resource?.resourceMetadata.iconURL, name: resource?.resourceMetadata.name, address: newAsset))
		...
		case let .resourcesLoaded(resources):
			...
			state.loadedResources = state.allDepositorAddresses.map { address in
				...
				return .init(
					iconURL: resourceDetails?.resourceMetadata.iconURL,
					name: resourceDetails?.resourceMetadata.name,
					address: address
				)
			}
			return .none
