...
extension ManifestBuilder {
	...
	static func createNonFungibleTokensRawManifest(account: AccountAddress, nrOfTokens: Int = 1) -> String {
		let instructions = {
			if nrOfTokens == 1 {
				return createMultipleIdsNonFungibleResourceRawInstruction
			} else {
				return Array(repeating: createSmallNonFungbileResourceRawInstruction, count: nrOfTokens).joined(separator: "\n")
			}
		}()
