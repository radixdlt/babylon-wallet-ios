@testable import AggregatedValueFeature
import FeaturePrelude
import TestUtils

final class AggregatedValueFeatureTests: TestCase {
	/*
	 func testVisibilityButtonTapped() {
	 	let store = TestStore(
	 		initialState: Home.AggregatedValue.State(value: 1000, currency: .usd, isCurrencyAmountVisible: false),
	 		reducer: Home.AggregatedValue.reducer,
	 		environment: Home.AggregatedValue.Environment()
	 	)

	 	store.send(.internal(.user(.toggleVisibilityButtonTapped))) {
	 		$0.isCurrencyAmountVisible = true
	 	}

	 	store.send(.internal(.user(.toggleVisibilityButtonTapped))) {
	 		$0.isCurrencyAmountVisible = false
	 	}
	 }
	 */
}
