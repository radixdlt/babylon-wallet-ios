...
final class ValueEncodingTests: TestCase {
	...
	func test_value_encoding_and_decoding() throws {
		let resourceAddress = try ResourceAddress(validatingAddress: "resource_sim1tknxxxxxxxxxradxrdxxxxxxxxx009923554798xxxxxxxxxakj8n3")
		let componentAddress = try ComponentAddress(validatingAddress: "component_sim1cqvgx33089ukm2pl97pv4max0x40ruvfy4lt60yvya744cvemygpmu")
		let packageAddress = try PackageAddress(validatingAddress: "package_sim1p4r4955skdjq9swg8s5jguvcjvyj7tsxct87a9z6sw76cdfd2jg3zk")

		let enumValues: [ManifestASTValue] = EnumDiscriminator.KnownStringDescriminators.allCases.map {
			.enum(.init(.string($0), fields: [.i8(10)]))
		} + [.enum(.init(.u8(10), fields: [.i8(10)]))]

		let values: [ManifestASTValue] = try [
			.boolean(true),
			.boolean(false),
			...
			.i32(10),
			.i64(10),
			.i128(.init(stringLiteral: "20")),

			.u8(10),
			.u16(10),
			.u32(10),
			.u64(10),
			.u128(.init(stringLiteral: "20")),
			...
			.array(.init(elementKind: .i8, elements: [.i8(10)])),
			.tuple(.init(values: [.string("sd"), .i8(10), .u8(10)])),
			.map(.init(
				keyKind: .string,
				valueKind: .i8,
				entries: [[.string("sd"), .i8(10)]]
			)),
			.decimal(.init(value: "10")),
			.preciseDecimal(.init(floatLiteral: 2.3)),

			.address(.init(validatingAddress: "resource_sim1tknxxxxxxxxxradxrdxxxxxxxxx009923554798xxxxxxxxxakj8n3")),
			.address(.init(validatingAddress: "component_sim1cqvgx33089ukm2pl97pv4max0x40ruvfy4lt60yvya744cvemygpmu")),
			.address(.init(validatingAddress: "package_sim1p4r4955skdjq9swg8s5jguvcjvyj7tsxct87a9z6sw76cdfd2jg3zk")),
			.bucket(.init(value: "bucket")),
		] // + enumValues

		let instructions: [any InstructionProtocol] = [
			TakeFromWorktop(resourceAddress: resourceAddress, bucket: .init(value: "bucket")),
			CallMethod(receiver: componentAddress, methodName: "aliases", arguments: values),
		]

		let manifest = TransactionManifest(instructions: .parsed(instructions.map { $0.embed() }))

		let result = RadixEngine.instance.convertManifest(request: .init(manifest: manifest, outputFormat: .string, networkId: .simulator))
		...
		let testVectors: [(value: ManifestASTValue, jsonRepresentation: String)] = [
			...
			//			),
			//			(
			//                                value: .address(componentAddress.asGeneral()),
			//                                jsonRepresentation:
			//                                """
			//                                {"type":"Address","address":"\(componentAddress.address)"}
			//                                """
			//                        ),
			//			(
			//                                value: .address(resourceAddress.asGeneral()),
			//				jsonRepresentation:
			//    """
			//    {"type":"Address","address":"\(resourceAddress.address)"}
			//    """
			//			),
			//			(
			//                                value: .address(packageAddress.asGeneral()),
			//				jsonRepresentation:
			//    """
			//     {"type":"Address","address":"\(packageAddress.address)"}
			//     """
			//			),
			//			(
			...
			//			),
			//			(
			//				value: .nonFungibleLocalId("241008287272164729465721528295504357972"),
			//				jsonRepresentation: """
			//				{"type":"NonFungibleLocalId","value":"241008287272164729465721528295504357972"}
			//				"""
			//			),
			//			(
			//				value: .nonFungibleLocalId("1"),
			//				jsonRepresentation: """
			//				{"type":"NonFungibleLocalId","value":{"type":"Integer","value":"1"}}
			//				"""
			//			),
			//			(
			//				value: .nonFungibleLocalId("Scrypto"),
			//				jsonRepresentation: """
			...
			////					nonFungibleLocalId: "241008287272164729465721528295504357972"
			////				)),
			////				jsonRepresentation: """
