...
extension DataChannelClient.Message {
	...
	var packageType: PackageType {
		switch self {
		case .chunkedMessage(.metaData): .metaData
		case .chunkedMessage(.chunk): .chunk
		case .receipt(.receiveMessageConfirmation): .receiveMessageConfirmation
		case .receipt(.receiveMessageError): .receiveMessageError
		}
	}
