...
extension DAppsDirectory {
	...
	struct AuthorizedDappsFeature: Sendable, FeatureReducer {
		...
		func reduce(into state: inout State, viewAction: ViewAction) -> Effect<Action> {
			...
			case .pullToRefreshStarted:
				guard !state.categorizedDApps.isLoading else {
					return .none
				}
				return loadAuthorizedDapps(&state, forceRefresh: true)
			}
		}
		...
		func reduce(into state: inout State, internalAction: InternalAction) -> Effect<Action> {
			...
			case let .loadedDapps(.failure(error)):
				state.categorizedDApps.refresh(from: .failure(error))
		...
		private func loadAuthorizedDapps(_ state: inout State, forceRefresh: Bool = false) -> Effect<Action> {
			state.categorizedDApps.refresh(from: .loading)
			return .run { send in
				let result = await TaskResult {
					let authorizedDapps = try await authorizedDappsClient.getAuthorizedDapps()
					let dAppsList = await (try? dAppsDirectoryClient.fetchDApps(forceRefresh)) ?? []
