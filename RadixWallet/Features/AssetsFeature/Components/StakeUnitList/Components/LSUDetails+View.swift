	...
	var viewState: LSUDetails.ViewState {
		.init(
			containerWithHeader: .init(
				title: .success(stakeUnitResource.resource.metadata.name ?? L10n.Account.PoolUnits.unknownPoolUnitName), // FIXME: ???
			...
			resourceDetails: .init(
				...
				resourceName: .success(stakeUnitResource.resource.metadata.title), // FIXME: ???
...
extension ValidatorHeaderView.ViewState {
	init(
		...
		self.init(
			...
			name: validator.metadata.name ?? L10n.Account.PoolUnits.unknownValidatorName, // FIXME: ???
			stakedAmount: nil
		)
	}
}
