...
extension DerivationPurpose {
	public var _description: String {
		switch self {
		case let .publicKeyForAddress(addressKind): "DerivationPurpose(.publicKeyForAddress(\(addressKind))"
		case .hashOfPublicKeyAsID: "DerivationPurpose(.hashOfPublicKeyAsID)"
		}
	}
...
extension DerivationPurpose {
	public static func publicKeyForAddressOfEntity<Entity: EntityProtocol>(type _: Entity.Type) -> Self {
		...
		case .account: .publicKeyForAddress(kind: .account)
		case .identity: .publicKeyForAddress(kind: .identity)
		}
	}
}
