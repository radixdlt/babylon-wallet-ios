...
extension HD.Path.Component {
	...
	public static func inferredDepth(string: String) throws -> Self {
		...
		case rootOnlyPublicKey:
			.root(onlyPublic: true)
		default:
			try .child(HD.Path.Component.Child(
	...
	public func toString() -> String {
		switch self {
		case let .child(childComponent): childComponent.toString()
		case let .root(onlyPublic): onlyPublic ? Self.rootOnlyPublicKey : Self.rootPrivateKey
		}
	}
	...
	public var asChild: Child? {
		switch self {
		case let .child(childComponent): childComponent
		case .root: nil
		}
	}
}
