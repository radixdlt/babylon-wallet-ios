...
extension HD.Path {
	...
	public var components: [Component] {
		switch self {
		case let .relative(relative): relative.components
		case let .full(full): full.components
		}
	}

	public func toString() -> String {
		switch self {
		case let .relative(relative): relative.toString()
		case let .full(full): full.toString()
		}
	}
...
//					return HD.Path.Component.Child.Depth.Value(self.components.count)
	...
	public struct Full: Hashable, Sendable, HDPathConvertible {
		public var onlyPublic: Bool {
			switch components[0] {
			case let .root(onlyPublic): onlyPublic
