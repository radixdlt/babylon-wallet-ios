...
extension HD.ExtendedKey {
	public enum Version: UInt32 {
		...
		var isPublic: Bool {
			switch self {
			case .mainnetPublic, .testnetPublic: true
			case .mainnetPrivate, .testnetPrivate: false
			}
		}

		var isPrivate: Bool {
			switch self {
			case .mainnetPublic, .testnetPublic: false
			case .mainnetPrivate, .testnetPrivate: true
			}
		}
	}
